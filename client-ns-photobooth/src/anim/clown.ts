...
import clownImg from '../assets/Clown/killer_clown.png'
...
// not just span the raw ear-to-ear distance.
 ...
 * face points already present in body pose are reused, same as how
 * calculateOwlSize/calculateBatSize derive a head-scale from ear distance. */
...
export async function createClownAnim(app: PIXI.Application) {
  const {
  ...
  const { texture } = await PIXI.ensureLoaded(loader, clownImg)
